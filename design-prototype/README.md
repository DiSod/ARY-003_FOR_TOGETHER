...
| Work Page | 表达单个作品资产、公开 Evidence 摘要与回到 Race 的上下文入口。 |
...
| Race Console / Organizer View | 展示 Organizer 入口和高密度办赛控制台。 |
| Rider View | 展示报名状态、CA 状态、骑行状态、作品提交、Review Result 和 Rider Report 入口。 |
| Judge View | 展示 Assigned Works、Evidence Summary 和评审动作入口。 |
| Admin Console | 展示 Users、Profile Completion 和 User.roles 管理。 |
| Screen Console | 展示 Race Selection、Display Mode、Theme / Calibration 和 Output Control。 |
| Screen Display | 展示大屏输出，并把 Screen Console 控制面分开。 |
| State Samples | 集中承载空态、错误态、权限感知异常摘要和 fallback 样张。 |
...
Home / Race Gallery
→ Race Page / Live Hall
→ Work / Work Detail / Results / Rider 资产入口
→ Screen Display
```
...
```text
Console
→ Rider View / Judge View / Admin Console
→ Screen Console
→ Screen Display
→ State Samples
```
...
本轮已补充移动端关键走查视口 `430 x 932`，用于验证首页、Rider View、State Samples 和 Screen Display 在窄屏下仍可读、可截图、可评审。
...
`State Samples` 的首屏概览图只用于说明该页面存在；当前用于空态 / 错误态走查的正式截图证据应优先使用按样张聚焦导出的截图集：
...
```text
docs/ux-1-annotation-matrix.md
```

该文档集中标注组件、关键状态、空态、错误态、权限差异和数据依赖，用于后续 `M2` 输入讨论。

当前 `State Samples` 页面作为空态 / 错误态集中样张入口，补足了此前只在文档中列出、但尚未被原型显式承载的体验状态。
