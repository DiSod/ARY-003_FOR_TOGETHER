...
const stateSampleCards = document.querySelectorAll("[data-state-sample-id]");
let currentHomeRaceId = sampleData?.raceGroups?.featuredRaceId;
let homeRaceCarouselTimer;

function getHashState() {
  const hashValue = location.hash.replace(/^#/, "");
  const [pageName, sampleId] = hashValue.split("/");
  return {
    pageName,
    sampleId,
  };
...
function stateSampleExists(sampleId) {
  return Array.from(stateSampleCards).some((card) => card.dataset.stateSampleId === sampleId);
}

function applyStateSampleFocus(sampleId) {
  const stateSamplesPage = document.querySelector('[data-page-panel="state-samples"]');
  if (!stateSamplesPage) return;
  const validSampleId = stateSampleExists(sampleId) ? sampleId : "";
  stateSamplesPage.classList.toggle("focused", Boolean(validSampleId));
  stateSamplesPage.dataset.stateSampleFocus = validSampleId;
  stateSampleCards.forEach((card) => {
    const isMatch = card.dataset.stateSampleId === validSampleId;
    card.classList.toggle("state-sample-card--focused", isMatch);
    card.hidden = Boolean(validSampleId) && !isMatch;
  });
}

function pageExists(pageName) {
  return Array.from(pagePanels).some((panel) => panel.dataset.pagePanel === pageName);
}

function setPage(pageName, sampleId = "") {
  ...
  const nextHash = sampleId ? `#${pageName}/${sampleId}` : `#${pageName}`;
  if (location.hash !== nextHash) {
    history.replaceState(null, "", nextHash);
  }
  applyStateSampleFocus(pageName === "state-samples" ? sampleId : "");
...
function renderPrototypeData() {
  ...
  html(
    ...
    selectedWorks
      .map((work, index) => {
        ...
        return `<article class="work-card ${index === 0 ? "hero-work" : ""}"><span>${index === 0 ? "精选作品" : statusText(race.status)}</span><h2>${escapeHtml(work.title)}</h2><p>${escapeHtml(work.summary)}</p><b>${escapeHtml(race.title)} / ${escapeHtml(work.status)}</b><button type="button" data-page="work-detail">查看详情</button></article>`;
      })
      .join(""),
  );
  ...
  html(
    ".ops-table",
    `<div><b>Rae Stone</b><span>Idle risk</span><em>review flag: no summary for 42m</em></div>
     <div><b>Ivy Sun</b><span>Evidence gap</span><em>needs judge attention</em></div>
     <div><b>Jun Park</b><span>Cost watch</span><em>monitor before review</em></div>
     <div><b>Ana Ruiz</b><span>Demo submitted</span><em>LocalJoy Agent ready</em></div>`,
  );
...
setPage(
  pageExists(initialHashState.pageName) ? initialHashState.pageName : "home",
  initialHashState.pageName === "state-samples" ? initialHashState.sampleId : "",
);
restartHomeRaceCarousel();
resizeAll();
...
window.addEventListener("hashchange", () => {
  ...
  setPage(pageExists(hashState.pageName) ? hashState.pageName : "home", hashState.pageName === "state-samples" ? hashState.sampleId : "");
});
requestAnimationFrame(drawFrame);
