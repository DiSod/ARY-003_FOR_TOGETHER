...
* `PRD-TEMP-1` 复审交付件 `prd-temp-1-baseline-delivery.md` 作为报名 / RaceProject / CA 参赛语义整改的并基线证明材料。
* `PRD-1` 正式交付件 `prd-1-delivery.md` 作为本任务的验收记录、未完成项和后续判断归档文件。
...
* `PRD-TEMP-1` 已完成复审收口，正式语义以 `prd-temp-1-baseline-delivery.md` 为并基线交付依据。
* PRD 能作为架构设计入口。
* `prd-1-delivery.md` 已完整记录本任务的可审查产物、验收记录、未完成项和后续判断。
...
* DCR Desktop App 设备注册、消息签名、验签、防重放和隔离审计。
...
* 未通过 DCR Desktop App 设备身份校验、消息签名校验或防重放校验的实时消息不进入 Projection、Evidence、Report 或评审摘要，只进入隔离审计。
