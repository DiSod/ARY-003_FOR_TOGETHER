...
* 所有测试必须遵守实时 CA 接入规则：CA 接入状态不作为参赛资格硬门禁；未登记、未握手、归属错误或被禁用的 CA 数据不得污染 Projection、Evidence 或 Report；不接受事后上传 Session Summary 伪造实时 CA 证据；比赛中的实时消息必须来自已登记的 DCR Desktop App，并通过设备身份、消息签名、时间戳 / nonce / sequence 校验。
...
* Public 不能访问真实性校验失败状态、隔离审计摘要或隔离审计明细。
* Public 不能访问 `rider_report`，除非后续单独增加公开发布规则。
* Rider 不能查看其他选手的非公开 Session、私有 Evidence、未发布评分和报告。
* Rider 只能查看自己的 `view_authenticity_status` 结果和必要异常摘要，不能查看其他选手的隔离审计摘要或明细。
* Rider / Judge 不能查看未发布 Award / Leaderboard draft。
* Judge 不能查看未分配给自己的作品，不能越权评分。
* Judge 只能在分配作品上下文中查看必要真实性风险摘要，不能访问隔离审计明细。
* Organizer 只能管理自己负责的 Race。
* Organizer 可以查看 managed race 范围内的 `view_authenticity_status`、`view_quarantine_audit_summary`，并且仅在内部异常处理场景访问 `view_quarantine_audit_detail`。
* Admin 可以维护 `User.roles`，但 Admin Console 不承担赛事执行、CA 接入维护或数据运营职责。
* Admin 可以访问真实性状态和隔离审计细节，但越权读取仍必须留下审计记录。
* 非 Admin 不能修改 `User.roles`。

验收点：

* 每个资源动作必须符合 `ary-permission-matrix.md`。
* `view_authenticity_status`、`view_quarantine_audit_summary`、`view_quarantine_audit_detail` 必须分别覆盖允许、拒绝和跨赛事越权三类测试。
...
* 已登记 CAConnection 通过未登记 DCR Desktop App、错误 deviceKeyId、错误签名、过期 timestamp、重复 nonce、sequence 回退或被篡改消息体上报。
* Rider、Judge、Organizer、Admin 分别访问 `view_authenticity_status`、`view_quarantine_audit_summary`、`view_quarantine_audit_detail` 的接口和页面边界。
...
* 验签失败、来源未知、时间戳异常、nonce 重放、sequence 回退或消息体摘要不一致的实时消息不得进入 Projection、Evidence、Report 或评审摘要，只能进入隔离审计。
* 真实性异常消息进入隔离审计后，Rider / Judge / Organizer / Admin 看到的摘要字段必须符合权限矩阵，且 Public 永远不可见。
