...
当前目的：基于现有 IA 对齐版高保真原型完成 UX-1 收口，并明确哪些设计资产进入 `M2` 架构输入。
...
当前轮次以“已有高保真原型评审收口”为目标，不从零开始重做视觉方向。

本轮优先完成：

1. 复审现有 `design-prototype/` 是否覆盖 `docs/ary.plan.md` 中 UX-1 要求的关键体验面：Public Site、Race Console、Admin Console、Screen Console、Screen Display。
2. 确认公众、Rider、Organizer、Judge、Admin、Screen Operator 的 P0 主路径是否都能被高保真页面走查。
3. 确认页面状态、权限入口、风险提示和真实性状态表达不违反当前产品 / 领域 / 权限基线。
4. 标记哪些页面和状态已经可作为 `M2` 架构设计输入，哪些仍属于视觉探索或待补页面。

本轮不做：

* 不推翻当前 1080P 高密度蓝白竞赛风格方向，除非评审发现其与 IA 或产品约束冲突。
* 不把 UX-1 扩大成完整前端规格书、接口文档或全量状态机文档。

本轮额外护栏：

* 页面中涉及 CA 接入异常时，应表达为证据缺口、来源异常、`verification_failed` 或 `quarantined`，不能误导为自动退赛。
* Public 不展示隔离审计细节；Rider 只展示自己的真实性状态结果；Organizer / Admin 可见异常摘要的设计边界不能被高保真误画反。
* Live Hall、过程榜和 Screen Display 可以消费 Projection，但不能把 Projection 画成最终事实源。
