...
| CA Message Authenticity | 比赛中的实时 CA 消息必须来自真实 CA，并通过受信接入端转发到 ARY | 实时骑行消息只能由已登记的 DCR Desktop App 代表对应 CAConnection 上报；ARY 必须验证设备身份、消息签名、时间戳 / nonce / sequence 防重放，并拒收或隔离验签失败、来源不明、被篡改或重放的消息 |
...
* 比赛中的实时 CA 消息必须来自已登记的 DCR Desktop App，并与对应 CAConnection 绑定；ARY 必须校验设备身份、消息签名、时间戳、nonce 和 sequence，防止伪造、篡改和重放。
* MVP 不接受事后手动上传 Session Summary 伪造实时 CA 证据；如作为说明材料引用，必须标记来源、时间和可信度。
* GitHub 只作为登录来源、作品代码入口或 Evidence 外部材料引用，不能替代实时 CA 接入。
* 验签失败、来源未知、时间戳异常、nonce 重放或签名体被篡改的实时消息不得进入 Projection、Evidence、Report 或评审摘要，只能进入隔离审计和 Organizer / Admin 异常视图。
...
| Review Flag | 评审前风险提示和材料完整度检查，用于提示空骑行、无 CA 数据、空作品、缺材料、疑似违规和接入异常 | `ary-domain-analysis.v0.3.md`、`ary-mvp.ia.md` |
...
| Review Flag | 驱动评审前风险提示、材料完整度提示和 Judge 评审上下文 | 空骑行、无 CA 数据、空作品、缺必填材料、疑似违规和接入异常应提示 Organizer / Judge，但不自动替代人工评审 |
