...
| CA Message Authenticity Status | CA 消息真实性状态 | verified、verification_failed、quarantined；表达 DCR Desktop App 设备身份与消息验签结果，失败消息不得进入有效比赛事实 |
...
| Review Flag | 评审前风险提示 | 标记空骑行、无 CA 数据、空作品、缺必填材料、疑似违规和接入异常，供 Organizer / Judge 处理和参考 |
...
| CA Message Authenticity Status | CA Connection / Session | DCR Desktop App 上报消息的真实性校验状态；verified 可进入有效证据链，verification_failed / quarantined 只进入隔离审计 |
...
| CA Connection Registration Flow | Race Project, CA Connection, CA Connector | 参赛过程中通过 DCR Desktop App / ARY CA Connector 登记和握手 CAConnection，形成可追溯的连接记录，并绑定设备身份与公钥 |
| CA Data Ingestion Flow | Race Project, CA Connection, Session, CA, Session Summary | 实时接入一个或多个已登记且握手成功 CAConnection 的 CA 骑行数据；消息必须通过 DCR Desktop App 验签、防重放校验后才能从实时 Session 生成 Session Summary |
| CA Message Verification Flow | DCR Desktop App, CA Connection, Riding Signal Message | 校验设备身份、消息签名、timestamp、nonce、sequence 和消息体摘要；失败消息进入隔离审计 |
...
| Review Flag Check | Registration, Race Project, Work, Evidence | 评审前识别空骑行、无 CA 数据、空作品、缺必填材料、疑似违规和接入异常，生成风险提示 |
...
* RaceProjectGenerated
* CAConnectionRegistered
* CAConnectionConnected
* CAConnectionFailed
* CAConnectionAcceptanceWindowUpdated
