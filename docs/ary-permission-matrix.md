...
| view_authenticity_status | - | own | assigned work summary | managed race | system |
| view_quarantine_audit_summary | - | own limited | assigned work risk summary | managed race | system |
| view_quarantine_audit_detail | - | - | - | managed race internal exception | system |
...
* 只有通过 DCR Desktop App 设备身份校验、消息签名校验和防重放校验的实时消息，才可进入 Projection、Evidence 或 Report 输入。
* 单个 CAConnection failed 或 RaceProject 聚合接入 failed 不触发 Registration 自动退赛，只形成连接异常、证据缺口和评审前风险提示。
* `view_authenticity_status` 仅暴露 `verified`、`verification_failed`、`quarantined` 及必要原因摘要，不直接暴露原始消息内容。
* `view_quarantine_audit_summary` 仅暴露与评审和运营相关的最小异常摘要，例如原因码、触发时间、影响连接和当前处置状态。
* Organizer / Admin 可查看真实性校验失败和隔离审计摘要；Rider 只能看到自己的真实性状态结果和必要异常摘要，不能查看其他选手的隔离审计细节。
* `view_quarantine_audit_detail` 属于内部异常处理权限，只允许 Organizer / Admin 在受控场景下查看。
