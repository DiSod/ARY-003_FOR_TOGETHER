...
| CA Message Authenticity | verified | 消息来源与签名校验通过；对应数据可进入正常证据链路 |
| CA Message Authenticity | verification_failed | 真实性校验失败；对应消息进入隔离审计，并在 Organizer / Rider View 中显示来源异常 |
| CA Message Authenticity | quarantined | 命中重放、签名不一致、来源异常或待人工确认；对应消息不进入正常证据链路 |
...
* Rider View 和 Organizer View 必须能看到 CAConnection 的真实性校验状态，例如 verified、verification_failed、quarantined；未通过校验的消息不进入正常证据链路。
...
* Organizer View 应提供来源异常提示，区分“无数据 / 接入异常”和“真实性校验失败 / 消息已隔离审计”。
* GitHub 只作为登录来源、作品代码入口或外部材料引用，不替代实时 CA 接入。
