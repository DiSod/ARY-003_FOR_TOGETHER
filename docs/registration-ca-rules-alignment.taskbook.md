...
* 比赛中的实时 CA 消息必须通过已登记的 DCR Desktop App 上报，并完成设备身份校验、消息签名校验和防重放校验。
* 未登记、归属错误、未握手或被禁用的 CAConnection push 不得直接进入 Projection、Evidence 或 Report 输入。
* 验签失败、来源不明、消息被篡改或被重放的实时消息不得进入 Projection、Evidence 或 Report 输入，只能进入隔离审计。
...
* 评审前风险提示文档命名已统一为 Review Flag；对应检查流程命名统一为 Review Flag Check。
...
| 事项 | 说明 |
| --- | --- |
| 评审前风险提示命名 | 已统一为 Review Flag；对应检查流程命名为 Review Flag Check |
| 提交准入边界 | 空作品、缺必填材料是允许提交并打 Review Flag，还是在提交前阻断，仍需在正式 Race Rules 中细化 |
| CAConnection 新增窗口 | 当前口径已收敛为 running / submitting 且尚未进入 judging 前可新增；需在正式 Race Rules 中固化 |
| 违规作品处理 | 系统提示、Organizer 标记、评审前人工处理的职责边界仍需后续细化 |
| UX / 高保真原型防回归 | 后续 Rider / Organizer / Judge 页面新增或改版时，需继续避免表达旧资格门禁 |

---

# 9. 当前结论

本任务已完成文档与高保真原型的一致性复审，建议作为 `PRD-1` 的已完成子任务并入正式文档基线。

后续 `DEV-5`、正式 Race Rules 和 UX 页面扩展应继续沿用本任务已确认的新口径，不得重新固化“注册阶段 / 开赛冻结 / CA 失败放弃参赛”的旧模型。
